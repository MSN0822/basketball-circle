...
export async function POST(req: NextRequest) {
  ...
  const { title, event_date, location, location_url = null, closes_at = null, publishes_at = null, max_participants = 40, threshold = 30, status = 'accepting' } = body

  if (!title || !event_date || !location) {
    return NextResponse.json({ error: 'title, event_date, location は必須です' }, { status: 400 })
  }

  const { data, error } = await supabase
    .from('events')
    .insert({ title, event_date, location, location_url, closes_at, publishes_at, max_participants, threshold, status })
    .select()
    .single()
...
export async function PATCH(req: NextRequest) {
  ...
  const body = await req.json()
  const { id, status, title, event_date, location, location_url, closes_at, publishes_at, max_participants, threshold } = body
