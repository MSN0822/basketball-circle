...
export async function GET(req: NextRequest) {
  ...
  const dormantList = (dormantMembers ?? []) as DormantMember[]

  for (let index = 0; index < dormantList.length; index += CLEANUP_BATCH_SIZE) {
    const batch = dormantList.slice(index, index + CLEANUP_BATCH_SIZE)
