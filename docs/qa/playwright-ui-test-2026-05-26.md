...
- `docs/qa/evidence/2026-05-27-playwright-QA_KEEP_UI_20260527032643/`
...
- `08-authenticated-event-detail-before-join.png` 参加者ログイン後のイベント詳細、参加前
- `09-authenticated-event-detail-after-join.png` 参加者ログイン後のイベント詳細、参加後
...
- 参加者としてログインし、イベント詳細を開ける。
- 参加者として参加ボタンを押し、参加後の画面証跡を取得できる。

## 残るUI観点

参加者本人としてログインした状態のUI操作は、`SUPABASE_SERVICE_ROLE_KEY` を使って `scripts/qa-provision-auth-user.mjs` で確認済みテスト会員を作成し、`.env.local` の `QA_AUTH_EMAIL` / `QA_AUTH_PASSWORD` に保存したうえで消化済み。
