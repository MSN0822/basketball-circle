...
- [status: open] [種別: 運用] [初出: 2026-07-02] [更新: 2026-07-02] DB バックアップの取得状況・復旧手順が RUNBOOK 未記載（Supabase 無料枠は自動物理バックアップなし・pg_dump 定期取得の運用検討）。Supabase 無料枠の自動休止（7日間アクセスなしで pause）の再開手順も未文書。Vercel cron の Hobby プラン制約（1日1回・実行遅延許容）・pg_cron 失敗の自動検知なしも未整理
