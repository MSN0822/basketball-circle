...
イベント一覧（トップページ）とイベント詳細ページのデータ取得が DB エラーで失敗した場合、「現在公開中のイベントはありません」や 404 ではなく、エラー画面（「情報を取得できませんでした」＋再読み込みボタン）が表示される。参加状況バッジの解決だけが失敗した場合は画面全体をエラーにせず、未ログイン相当の表示にフォールバックして詳細はサーバーログにのみ記録する。

- 根拠: `lib/event-queries.ts`, `app/error.tsx`, `app/page.tsx` の getEvents, `app/events/[id]/page.tsx` の getEvent / getParticipants
- 備考: 2026-07-21 の修正前は Supabase の error を無視して空配列・404 として描画していたため、DB 障害とイベント0件を区別できなかった（TASKS.md M-13）。
...
- 確認結果: 毎アクセス更新が望ましいが、24時間間引きは DB 書き込み削減のための合理的な実装であり、休眠判定（365日）へのずれは最大24時間＝実質影響ゼロのため**現状維持**。docs/operations-spec.md の「アクセスのたびに更新」という記述は実装に合わせて修正済み（2026-07-21）。関連: CRON-16。
...
### ADM-12 — ✅ 確認済み（2026-07-11 まっすん確認）

管理者画面のイベント詳細で「締め切る」ボタンを押すと status を closed に変更する（PATCH経由で is_manual_close=true が設定される）。「再開する」ボタンでは status を accepting に戻す（is_manual_close=false にリセットされる）。下書きの場合は「今すぐ公開」で accepting に変更する。archived の場合は状態変更ボタン・編集ボタン自体がUI上から非表示になり（handleToggleStatus のガードだけでなくボタン表示条件でも二重に保護）、PATCH API側もarchivedイベントへの変更リクエストを409で拒否する。

- 根拠: `app/admin/events/[id]/page.tsx:205-224, app/api/admin/events/route.ts:279-282`
...
### ADM-19 — ✅ 確認済み（2026-07-03 まっすん確認・2026-07-21 実装対応）

管理者画面のイベント編集（PATCH）で定員（max_participants）を小さくする変更は、現在の active 参加者数を**下回る値には設定できない**（400「現在の参加者数（N名）を下回る定員には変更できません」で拒否）。参加者数と**同数**に設定した場合は即時受付停止（closed）へ遷移する。定員を増やす変更・据え置きの再送信では参加者数の確認を行わない。

- 根拠: `app/api/admin/events/route.ts` PATCH の定員検証（archived 409 ガードと閾値検証の後、patch 組み立ての前）
- 確認結果: 想定どおりに実装（2026-07-21）。判定は**定員を減らす方向のときだけ**行う（編集画面が変更の有無に関わらず max_participants を毎回送信するため、無条件に検証すると既に「参加者数 > 定員」になっているイベントが編集不能になる）。同数到達による自動 closed では `is_manual_close` を false のままにする（true にすると EVT-08 / EVT-18 / JOIN-10 / ADM-22 の自動再開が永久に止まる）。`status='accepting'` が同時に送られた場合は closed を優先。
...
### ADM-23 — ✅ 確認済み（2026-07-11 まっすん確認）

アーカイブ済みイベントは編集・状態変更ができない。管理画面UIでは「編集」ボタン・状態変更ボタン（締め切る/再開する/今すぐ公開）自体が表示されず、PATCH /api/admin/events を直接呼び出した場合もサーバー側でイベントのステータスをチェックし409エラーで拒否する（画面とAPIの両方で保護）。イベント削除ボタンはarchivedでも引き続き表示・実行可能（アーカイブされたイベントの整理用途のため対象外）。
...
- 確認結果: 現状維持（MEM-08 と同一仕様の cron 視点。docs 側の記述修正は 2026-07-21 に対応済み）。
...
Supabase 接続に必要な環境変数（NEXT_PUBLIC_SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY）が欠落している場合、対象の API ルートは「サーバー設定エラーです」という 500 JSON を返す。環境変数名などの設定情報はレスポンス本文に含めず、詳細は console.error にのみ記録される。ルートのモジュール読み込み時点では例外を投げないため、ある環境変数の欠落が他の画面・API の起動まで巻き込むことはない。

- 根拠: `lib/route-supabase.ts:19-33`、`app/api/participants/route.ts` ほか 4 ルートの各ハンドラ冒頭
- 備考: 2026-07-21 に「モジュールトップでクライアント生成 → 環境変数欠落時は import 時点で throw（該当ルートが起動しない）」から現在の挙動へ変更した。回帰防止テストは `tests/unit/route-lazy-supabase.test.ts`（5 ルートの import と 10 ハンドラを網羅）。
